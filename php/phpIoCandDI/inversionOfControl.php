<?php
/* 控制反转 
 * 是面向对象编程中的一种设计原则，可以用来减低计算机代码之间的耦合度。其中最常见的方式叫做依赖注入（Dependency Injection, DI）,
 * 还有一种叫"依赖查找"（Dependency Lookup）。通过控制反转，对象在被创建的时候，
 * 由一个调控系统内所有对象的外界实体，将其所依赖的对象的引用传递给它。也可以说，依赖被注入到对象中。
 */
...
/* In this example, the PurchasePodcast job needs to send e-mails when a podcast is purchased. 
 * So, we will inject a service that is able to send e-mails. Since the service is injected, 
 * we are able to easily swap it out with another implementation. We are also able to easily "mock", 
 * or create a dummy implementation of the mailer when testing our application.
 */
 ...
 * 
 * 如果我们仔细研究了Service Container我们就会发现laravel的服务容器中只存储了对象的描述，而并不需要知道如何具体的去构造一个对象，
 * 因为它会根据php的反射服务去自动解析具体化一个对象。
 */

/* 反射
 * 在计算机科学中，反射是指计算机在运行时（Run time）可以访问、检测和修改它本身状态或行为的一种能力。用来比喻说，那种程序能够“观察”并且修改自己的行为。
 * 支持反射的语言提供了一些在低级语言中难以实现的运行时特性。这些特性包括 作为一个第一类对象发现并修改源代码的结构（如代码块、类、方法、协议等）。
 * 将跟class或function匹配的转换成class或function的调用或引用。 在运行时像对待源代码语句一样计算字符串。
 * 创建一个新的语言字节码解释器来给编程结构一个新的意义或用途。 PHP实现的反射可以在官网文档中进行查看： 反射API
 */ 
 ...
 * 其实。。。不是这样的。。。(@ο@)
 * 
 * 很多时候我们为了提高代码的扩展性和维护性，在编写类时依赖的是接口或抽象类，而并不是一个具体的实现类。明白了吗？
 * 依赖解析的时候如果只解析到接口或抽象类，然后利用反射，那么这个依赖肯定是错误的。
 * 
